...
with lib;
{
  ...
  options.programs = {
    vscode.continue.enable = mkEnableOption "Continue extension for Visual Studio Code";
    antigravity.continue.enable = mkEnableOption "Continue extension for Antigravity";
  };
  ...
  config =
      ...
      mkEditorConfig =
        {
          ...
          dataFolderName,
          configDirName,
          includeVscodeOnlyExtensions ? false,
          extraPersistenceDirectories ? [ ],
          extraPersistenceFiles ? [ ],
        }:
        let
        ...
        mkIf cfg.enable {
          home.persistence."/persist" = {
            ...
            ++ extraPersistenceDirectories;
            files = extraPersistenceFiles;
          };

          programs.${programName} = {
            package = if cfg.continue.enable then package |> overrideForContinue else package;
            ...
            profiles.default = {
              extensions =
                ...
                ++ optionals includeVscodeOnlyExtensions [
                  # LLM Assistant
                  github.copilot-chat
    ...
    mkMerge [
      (mkEditorConfig {
        ...
        configDirName = "Code";
        includeVscodeOnlyExtensions = true;
      })
      (mkEditorConfig {
        ...
        extraPersistenceFiles = [ ".gemini/GEMINI.md" ];
      })
      (mkIf (config.programs.vscode.continue.enable || config.programs.antigravity.continue.enable) {
        home.persistence."/persist".directories = [ ".continue" ];
      })
    ];
}
